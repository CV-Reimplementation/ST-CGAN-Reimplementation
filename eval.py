...
def parse_args():
    ...
    parser.add_argument('--netG1', default='./output/G1_epoch_300.pth', help="path to netG1 (to continue training)")
    parser.add_argument('--netD1', default='./output/D1_epoch_300.pth', help="path to netD1 (to continue training)")
    parser.add_argument('--netG2', default='./output/G2_epoch_300.pth', help="path to netG2 (to continue training)")
    parser.add_argument('--netD2', default='./output/D2_epoch_300.pth', help="path to netD2 (to continue training)")
...
def getLoader(dataroot, originalSize, imageSize, batchSize=64, workers=8,
...
def main(opt):
    ...
    for i, val_cpu in enumerate(tqdm(valDataloader)):
        filename = val_cpu['filename'][0]
        single_img = Variable(val_cpu['imgA'].type(Tensor))
        with torch.no_grad():
            val_inputv = Variable(single_img)
        fake_B = G1(val_inputv)
        fake_C = G2(torch.cat([single_img, fake_B], 1))
        vutils.save_image(fake_C.data, '%s/val_results/%s' % (opt.output_dir, filename), normalize=True)
