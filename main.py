    ...
    for epoch in range(opt.niter):
        ...
        for i, data in enumerate(tqdm(dataloader), 0):
            ...
            fake_ABC = torch.cat((real_A, fake_B, fake_C),
                                 1)  # we use conditional GANs; we need to feed both input and output to the
            # discriminator
            ...
            #     print_current_losses(os.path.join(opt.output_dir, 'train.log'), epoch, lr, epoch_iter, losses, t_comp,
            #                          t_data)
            
            

        if epoch % opt.evalIter == 0:  # evaluation
